...
Module MODULOFILTRAR
    ...
    Sub filtrarPorFechaDeCompetencia(ByVal fecha As Date, ByVal dgvAtletasxCompetencia As DataGridView)
        Try
            adaptador = New SqlDataAdapter("select * from Competencia where Fecha_Inicio '" & fecha + "%" & "'", cn)
            dt = New DataTable
            adaptador.Fill(dt)
            dgvAtletasxCompetencia.DataSource = dt
        Catch ex As Exception
        ...
        End Try
    End Sub
    'Dim Ssql As String = "select * from fecha where   FechaAsignacion >= '" & Format(DateTimePicker1.Value.Date.ToString, "short date") & _
    '   " ' and FechaAsignacion <= ' " & Format(DateTimePicker2.Value.Date.ToString, "short date") & "' "
    

    Sub filtrarPorSexo(ByVal sexo As Integer, ByVal dgvListaAtleta As DataGridView)
        Try
            adaptador = New SqlDataAdapter("select * from Sexo where Sexo '" & 2 + "%" & "'", cn)
            dt = New DataTable
            adaptador.Fill(dt)
            dgvListaAtleta.DataSource = dt
        Catch ex As Exception
            MessageBox.Show("error al realizar el filtrado " + ex.ToString)
        End Try
    End Sub
